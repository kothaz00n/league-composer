...
function getRecommendations({
    ...
    const targetPoolSet = new Set(
        targetArchetypeDef?.champion_pool?.[normalizedRole]
    ...
    if (targetArchetypeDef && targetArchetypeDef.typical_comp) {
        ...
        if (planNameRaw) {
            plannedName = cleanName(planNameRaw);
            isPlannedFlex = planNameRaw.endsWith('*');
        }
    }
    ...
    for (const champId of Object.keys(allChampions)) {
        ...
        if (targetPoolSet.has(champName)) {
            score += 8;
