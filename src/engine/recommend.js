...
function getRecommendations({
    ...
    // Pre-calculate loop-invariant computations for custom archetypes
    ...
    if (targetArchetypeDef) {
        if (targetArchetypeDef.champion_pool) {
            ...
            if (normalizedRole && targetArchetypeDef.champion_pool[normalizedRole]) {
                customArchetypePool = targetArchetypeDef.champion_pool[normalizedRole].map(n => cleanName(n));
            }
        }
        if (targetArchetypeDef.typical_comp && normalizedRole) {
            const planNameRaw = targetArchetypeDef.typical_comp[normalizedRole];
            if (planNameRaw) {
                customArchetypePlanName = cleanName(planNameRaw);
                customArchetypePlanIsFlex = planNameRaw.endsWith('*');
            }
        }
    }

    // Pre-calculate flex mode allies
    let flexAlliesByRole = {};
    ...
    for (const champId of Object.keys(allChampions)) {
        ...
        if (customArchetypePlanName && customArchetypePlanName === champName) {
        ...
        if (customArchetypePool && customArchetypePool.includes(champName)) {
            score += 8;
        ...
        if (rosterConfig && rosterConfig.roster) {
            ...
            if (rosterConfig.gameMode === 'flex') {
                ...
                for (const r in flexAlliesByRole) {
...
function getCompositionAnalysis(teamRoles, queue = 'soloq') {
    ...
    // Pre-calculate current team member names for O(1) lookup
