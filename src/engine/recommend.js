...
function getRecommendations({
    ...
    // Memoize loop-invariant calculations for archetype roles
    const derivedArchetypeRoles = (targetArchetypeDef && targetArchetypeDef.champion_pool)
        ? deriveRolesFromPool(targetArchetypeDef.champion_pool)
        : null;

    // Memoize loop-invariant custom champion pool
    const customChampionPool = (targetArchetypeDef && targetArchetypeDef.champion_pool && targetArchetypeDef.champion_pool[normalizedRole])
        ? targetArchetypeDef.champion_pool[normalizedRole].map(n => cleanName(n))
        : null;

    // Memoize loop-invariant flex synergy parameters
    let flexSynergyAllies = null;
    if (rosterConfig && rosterConfig.gameMode === 'flex' && rosterConfig.roster) {
        ...
        for (const r of otherRoles) {
            ...
            if (allyInRole && allyInRole.championId === 0) {
                flexSynergyAllies.push({ role: r, favorites: rosterConfig.roster[r].favorites || [] });
            }
        }
    }
    ...
    for (const champId of Object.keys(allChampions)) {
        ...
        if (customChampionPool && customChampionPool.includes(champName)) {
            score += 8;
        ...
        if (rosterConfig && rosterConfig.roster) {
            ...
            // Flex Mode Synergy
            if (flexSynergyAllies) {
                let flexSynergyParams = 0;

                for (const ally of flexSynergyAllies) {
                    for (const fav of ally.favorites) {
