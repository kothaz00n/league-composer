...
function getRecommendations({
    ...
    // ─── Loop Invariant Pre-calculations ─────────────────────────────────
    // 1. Custom Archetype derived roles
    ...
    // 2. Custom Archetype champion pool for this role (O(1) lookup)
    let roleChampionPoolSet = null;
    if (targetArchetypeDef && targetArchetypeDef.champion_pool && targetArchetypeDef.champion_pool[normalizedRole]) {
        roleChampionPoolSet = new Set(targetArchetypeDef.champion_pool[normalizedRole].map(n => cleanName(n)));
    }

    // 3. Flex Synergy Target Favorites
    // If in flex mode, we want to know what allies still need a pick and what their favorites are.
    let flexTargetFavs = [];
    ...
    for (const champId of Object.keys(allChampions)) {
        ...
        if (roleChampionPoolSet && roleChampionPoolSet.has(champName)) {
            score += 8;
        ...
        if (rosterConfig && rosterConfig.roster) {
            ...
            // Flex Mode Synergy
            if (rosterConfig.gameMode === 'flex' && flexTargetFavs.length > 0) {
                let flexSynergyParams = 0;
                for (const fav of flexTargetFavs) {
