...
function getRecommendations({
    ...
    // Pre-calculate custom archetype derived roles if present
    let customArchetypeDerivedRoles = null;
    if (targetArchetypeDef?.champion_pool && !targetArchetype) {
        customArchetypeDerivedRoles = deriveRolesFromPool(targetArchetypeDef.champion_pool);
    }

    // Pre-calculate normalized champion pool for current role
    let normalizedChampPoolForRole = null;
    if (targetArchetypeDef?.champion_pool?.[normalizedRole]) {
        normalizedChampPoolForRole = new Set(targetArchetypeDef.champion_pool[normalizedRole].map(n => cleanName(n)));
    }

    // Pre-calculate plan Name
    let planNameExact = null;
    let planNameIsFlex = false;
    if (targetArchetypeDef && targetArchetypeDef.typical_comp) {
        const planNameRaw = targetArchetypeDef.typical_comp[normalizedRole];
        if (planNameRaw) {
            planNameExact = cleanName(planNameRaw);
            planNameIsFlex = planNameRaw.endsWith('*');
        }
    }

    // Pre-calculate flex mode empty roles
    const flexEmptyRolesFavs = [];
    if (rosterConfig?.gameMode === 'flex' && rosterConfig.roster) {
        const otherRoles = Object.keys(rosterConfig.roster).filter(r => r !== normalizedRole);
        for (const r of otherRoles) {
            // Find if there's an ally in this role who hasn't picked yet
    ...
    for (const champId of Object.keys(allChampions)) {
        ...
        if (normalizedChampPoolForRole && normalizedChampPoolForRole.has(champName)) {
            score += 8;
        ...
        if (rosterConfig && rosterConfig.roster) {
            ...
            // Flex Mode Synergy
            if (flexEmptyRolesFavs.length > 0) {
                let flexSynergyParams = 0;
                for (const fav of flexEmptyRolesFavs) {
